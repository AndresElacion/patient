                    ...
                    <header class="flex flex-col space-y-6">
                        ...
                        <div class="flex gap-8 items-center">
                            ...
                            <div class="w-40 h-40 rounded-full overflow-hidden border-4 border-gray-300 shadow-md">
                                <img src="{{ asset('storage/' . $doctor->image) }}" alt="Doctor Image" class="w-full h-full object-cover">
                            </div>

                            <!-- Basic Details -->
                            <div class="space-y-3">
                                <h1 class="text-3xl font-bold text-gray-800">{{ $doctor->name }}<span class="inline-block w-4 h-4 ml-2 border border-green-600 bg-green-500 animate-pulse rounded-full"></span></h1>
                                <p class="text-lg text-gray-600">{{ $doctor->age }} Years, {{ $doctor->gender }}</p>
                                <p class="text-md text-gray-500">{{ $doctor->department }} - {{ $doctor->specialty }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                            ...
                            <div class="space-y-2">
                                ...
                                <p class="text-lg text-gray-800">{{ $doctor->contactNumber }}</p>
                            </div>
                            <div class="space-y-2">
                                <h3 class="text-sm text-gray-500 uppercase">Date of Birth</h3>
                                <p class="text-lg text-gray-800">{{ $doctor->dateOfBirth }}</p>
                            </div>
                            <div class="space-y-2">
                                <h3 class="text-sm text-gray-500 uppercase">Address</h3>
                                <p class="text-lg text-gray-800">{{ $doctor->address }}</p>
                            </div>
                        </div>
                    </header>
                </div>
